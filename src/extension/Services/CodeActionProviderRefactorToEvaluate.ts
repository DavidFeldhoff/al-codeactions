...
export class CodeActionProviderRefactorToEvaluate implements ICodeActionProvider {
    ...
    async createCodeActions(): Promise<CodeAction[]> {
        ...
        if (this.range.start.compareTo(this.range.end) == 0) {
            ...
            await this.changeAssignmentToEvaluateExpression(assignmentTreeNode, edit);
        } else {
            if (this.range.start.line == this.range.end.line) {
        ...
        if (edit.entries().length > 0) {
            let codeAction: CodeAction = new CodeAction('Refactor to Evaluate-Statement', CodeActionKind.RefactorRewrite);
    ...
    async changeAssignmentToEvaluateExpression(assignmentTreeNode: ALFullSyntaxTreeNode | undefined, edit: WorkspaceEdit): Promise<void> {
        ...
        edit.insert(this.document.uri, evaluatePosition, 'Evaluate(');
    }
}