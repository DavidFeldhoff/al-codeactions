...
import { TextDocument, Range, Diagnostic } from 'vscode';

export class CodeActionProviderCreateProcedureFactory {
    public static getInstances(document: TextDocument, range: Range): ICodeActionProvider[] {
        let diagnostics: Diagnostic[] = new DiagnosticAnalyzer().getValidDiagnosticOfCurrentPositionToCreateProcedure(document, range);
