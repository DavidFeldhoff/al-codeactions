...
export class CodeActionProviderModifyProcedureContent {
    ...
    async considerLine(): Promise<boolean> {
        ...
        let regex: RegExp = /^(\s+(?:local|internal|protected|public)? procedure )(\w+|"[^"]+")\(/i;
    ...
    async createCodeActions(): Promise<CodeAction[]> {
        ...
        if (!methodNode)
            return [];
        let eventNodes: ALFullSyntaxTreeNode[] = ALFullSyntaxTreeNodeExt.collectChildNodesOfKinds(methodNode, [FullSyntaxTreeNodeKind.getMemberAttribute()], false);
        if (eventNodes.some((eventNode) => eventNode.identifier && eventNode.identifier.toLowerCase() in ['integrationevent', 'businessevent', 'internalevent']))
            return [];
        let identifierNameSanitized: string | undefined = ALFullSyntaxTreeNodeExt.getIdentifierValue(this.document, methodNode, true);
        ...
        let createOnBeforeCodeAction: boolean = !this.document.getText(methodRange).includes(onBeforePublisherName + '(');
    ...
    async getWorkspaceEditComplete(publisherToAdd: PublisherToAdd, sourceLocation: Location, vscodeInstance: any = vscode): Promise<WorkspaceEdit | undefined> {
        ...
        let globalVarSections = ALFullSyntaxTreeNodeExt.collectChildNodesOfKinds(methodNode.parentNode!, [FullSyntaxTreeNodeKind.getGlobalVarSection()], false);
        ...
        let returnNode = ALFullSyntaxTreeNodeExt.getFirstChildNodeOfKind(methodNode, FullSyntaxTreeNodeKind.getReturnValue(), false);
        ...
        let blockNode = ALFullSyntaxTreeNodeExt.getFirstChildNodeOfKind(methodNode, FullSyntaxTreeNodeKind.getBlock(), false);
        if (!blockNode)
            return
        ...
        switch (publisherToAdd) {
            case PublisherToAdd.OnBefore:
                ...
                selection = await this.getWorkspaceEditForOnBeforePublisher(localVariables, parameters, returnVariable, globalVariables, methodNode, workspaceEdit, rangeOfBlockNode, publisherName, vscodeInstance);
                break;
            case PublisherToAdd.OnAfter:
                publisherName = 'OnAfter' + identifierNameSanitized;
                selection = await this.getWorkspaceEditForOnAfterPublisher(rangeOfBlockNode, methodNode, localVariables, parameters, returnVariable, lastExitStatementData, globalVariables, publisherName, workspaceEdit, vscodeInstance);
                break;
        }
        if (!selection)
            return undefined;
    ...
    async getWorkspaceEditForOnBeforePublisher(localVariables: ALVariable[], parameters: ALVariable[], returnVariable: ALVariable | undefined, globalVariables: ALVariable[], methodNode: ALFullSyntaxTreeNode, workspaceEdit: WorkspaceEdit, rangeOfBlockNode: Range, publisherName: string, vscodeInstance: any = vscode): Promise<ALVariable[] | undefined> {
        ...
        possibleParameters.push({ reason: 'IsHandled', variable: isHandledVariable, pickDefault: true, parameterPositionPrio: 10 });
        if (returnVariable) {
            if (!returnVariable.name) {
                let returnNode: ALFullSyntaxTreeNode = ALFullSyntaxTreeNodeExt.getFirstChildNodeOfKind(methodNode, FullSyntaxTreeNodeKind.getReturnValue(), false)!;
            ...
            if (returnVariable.name)
                if (!possibleParameters.some((possibleparam) => possibleparam.variable.name == returnVariable.name))
                    possibleParameters.push({ reason: 'return variable', variable: returnVariable, pickDefault: true, parameterPositionPrio: 9 });
        }
        for (const parameter of parameters)
            if (!possibleParameters.some((possibleparameter) => possibleparameter.variable.name == parameter.name))
                possibleParameters.push({ reason: 'parameter', variable: parameter, pickDefault: true, parameterPositionPrio: 8 });
        for (const localVariable of localVariables)
            if (!possibleParameters.some((possibleparameter) => possibleparameter.variable.name == localVariable.name))
                possibleParameters.push({ reason: 'local variable', variable: localVariable, pickDefault: false, parameterPositionPrio: 7 });
        ...
        if (!isHandledExists && isHandledSelected) {
            let textEdit: TextEdit = WorkspaceEditUtils.addVariableToLocalVarSection(methodNode, isHandledVariable, this.document);
    ...
    async getWorkspaceEditForOnAfterPublisher(rangeOfBlockNode: Range, methodNode: ALFullSyntaxTreeNode, localVariables: ALVariable[], parameters: ALVariable[], returnVariable: ALVariable | undefined, lastExitStatementData: { rangeOfExitStatement: Range; variable: ALVariable | undefined; exitContent: string; } | undefined, globalVariables: ALVariable[], onAfterPublisherName: string, onAfterWorkspaceEdit: WorkspaceEdit, vscodeInstance: any) {
        let possibleParameters: { reason: string, variable: ALVariable, pickDefault: boolean, parameterPositionPrio: number }[] = []
        ...
        if (returnVariable) {
            ...
            if (returnVariableIsRequiredButMissing) {
                let returnNode: ALFullSyntaxTreeNode | undefined = ALFullSyntaxTreeNodeExt.getFirstChildNodeOfKind(methodNode, FullSyntaxTreeNodeKind.getReturnValue(), false);
                returnVariable.name = await this.askForReturnVariableName(localVariables, parameters, globalVariables, vscodeInstance);
                if (returnVariable.name === undefined)
                    return undefined;
                if (returnVariable.name)
            ...
            if (returnVariable.name)
                if (!possibleParameters.some((possibleparam) => possibleparam.variable.name == returnVariable.name))
                    possibleParameters.push({ reason: 'return variable', variable: returnVariable, pickDefault: !lastExitStatementData, parameterPositionPrio: 9 });
        }
        for (const parameter of parameters)
            if (!possibleParameters.some((possibleparameter) => possibleparameter.variable.name == parameter.name))
                possibleParameters.push({ reason: 'parameter', variable: parameter, pickDefault: true, parameterPositionPrio: 8 });
