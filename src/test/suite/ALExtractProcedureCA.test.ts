...
import { TextDocument, workspace, window, Range, Position } from 'vscode';
...
suite('ALExtractProcedureCA Test Suite', function () {
    let codeunitToExtractDocument: TextDocument;
    ...
    this.beforeAll('beforeTests', async function () {
        ...
        window.showInformationMessage('Start all tests of ALExtractProcedure.');
    });
    ...
    test('MultipleDeclarationsOfVariablesInOneLine', async () => {
        ...
        let alProcedure: ALProcedure | undefined = await new CodeActionProviderExtractProcedure(codeunitToExtractDocument, rangeToExtract).provideProcedureObjectForCodeAction(rangeToExtract, rangeToExtract, returnTypeAnalyzer);
    ...
    test('ExtractIfOneliner', async () => {
        ...
        let rangeToExtract: Range = getRange(codeunit1Document, procedureName, textToExtractStart, textToExtractEnd);
        ...
        await returnTypeAnalyzer.analyze();
        let alProcedure: ALProcedure | undefined = await new CodeActionProviderExtractProcedure(codeunit1Document, rangeToExtract).provideProcedureObjectForCodeAction(rangeToExtract, rangeToExtract, returnTypeAnalyzer);
        assert.notStrictEqual(alProcedure, undefined, 'Procedure should be extracted');
        alProcedure = alProcedure as ALProcedure;
        assert.strictEqual(alProcedure.accessModifier, AccessModifier.local);
        assert.strictEqual(alProcedure.returnType, undefined);
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (inProcedure) {
                ...
                if (textOfLine.includes(textToExtractStart)) {
                    startPos = new Position(i, document.lineAt(i).text.indexOf(textToExtractStart));
                }
                if (startPos) {
