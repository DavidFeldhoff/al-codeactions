...
import { CodeActionProviderRefactorToEvaluate } from '../../extension/Services/CodeActionProviderRefactorToEvaluate';
import { ALLanguageExtension } from '../alExtension';
import { ALTestProject } from './ALTestProject';

// import * as myExtension from '../extension';

suite('RefactorEvaluate Test Suite', function () {
	...
	this.beforeAll('beforeTests', async function () {
		...
		window.showInformationMessage('Start all tests of CodeActionProviderRefactorToEvaluate.');
	});
	...
	test('refactorByPosition', async () => {
		...
		let codeActions: CodeAction[] = await new CodeActionProviderRefactorToEvaluate(tmpDoc, new Range(positionToExecute, positionToExecute)).createCodeActions();
		...
		assert.strictEqual(lineText, 'Evaluate(MyField, MissingProcedureWithFieldReturn1(myInteger));', 'wrong refactoring to Evaluate')
	...
	test('refactorByRangeOfOneLine', async () => {
		...
		let tmpDoc: TextDocument = await workspace.openTextDocument({ language: 'al', content: codeunit1Document.getText() })
		let codeActions: CodeAction[] = await new CodeActionProviderRefactorToEvaluate(tmpDoc, rangeOfAssignmentStatement).createCodeActions();
		assert.strictEqual(codeActions.length, 1, 'Expected one codeAction');
		assert.notStrictEqual(codeActions[0].edit, undefined, 'workspaceedit expected')
		let edit = codeActions[0].edit as WorkspaceEdit;
		await workspace.applyEdit(edit);
	...
	test('refactorMultipleLines', async () => {
		...
		let tmpDoc: TextDocument = await workspace.openTextDocument({ language: 'al', content: codeunit1Document.getText() })
		let codeActions: CodeAction[] = await new CodeActionProviderRefactorToEvaluate(tmpDoc, rangeOfAssignmentStatements).createCodeActions();
		assert.strictEqual(codeActions.length, 1, 'Expected one codeAction');
		assert.notStrictEqual(codeActions[0].edit, undefined, 'workspaceedit expected')
		let edit = codeActions[0].edit as WorkspaceEdit;
		await workspace.applyEdit(edit);
		...
		assert.strictEqual(
			lineText,
			'Evaluate(MyField, MissingProcedureWithFieldReturn1(myInteger));\r\n' +
			'        Evaluate(MyField, MissingProcedureWithFieldReturn2(myInteger));\r\n' +
			'        Evaluate("MyField", MissingProcedureWithFieldReturn3(myInteger));',
			'wrong refactoring to Evaluate')
		await window.showTextDocument(tmpDoc)
		await commands.executeCommand('workbench.action.closeActiveEditor')
	});
