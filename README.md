...
|- *AA0206 (Assigned, but unused variables)*| This fix comes with a safe and an unsafe mode. In the safe mode I'll only remove assignments that are safe to delete, e.g. `a := b`. Assignments with function calls like `a := myProcedure()` won't be deleted as it could change the business logic if something happens inside `myProcedure()`. So I'd suggest to use the safe mode first and afterwards use the unsafe mode (if at all), but then double-check manually to make sure that no business logic has changed! |
