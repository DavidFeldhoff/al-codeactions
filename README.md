...
|**Extract to Label**|If you're inside a method or trigger and there's a string literal you can extract that one directly to a label. In case the label should be locked, then there's also a second code action that does it directly for you. And last but not least there's a config which allows you to explain placeholders if there are any in the text to be extracted. [Demo](#extract-to-label)|
