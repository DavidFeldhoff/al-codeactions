...
|**Remove all With-usages**     | Command "Remove all With-Usages": Microsoft makes the explicit and implicit use of a WITH statement obsolete. In the newest AL Language extension (insider version) they created diagnostics for that (AL0604 and AL0606). **This al language version (6.0.*) is necessary for this command.**  Unfortunately, you would have to go through each file and click the code action manually each time. This command fixes (nearly) all statements for you. But please make sure that you deactivate commands which are executed on save like the renaming/reorganizing of a file before running this command as they probably break the batch run.      |
